...
def listen(host, port):
    ...
    while True:
            ...
            for address in current_active_members:
                ...
                for user in all_members:
                    if int(user["personal_port"])!=int(address[1]):
                        active_members.append({"username": user["username"], "personal_port": user["personal_port"]})
                        print(active_members)
                    if int(user["personal_port"]) != int(address[1]) and address == addr:
                        active_users_for_current_user.append({"username": user["username"], "personal_port": user["personal_port"]})
        ...
        if payload == 'end_session:':
            err = requests.delete(f'{localhost}/client/{client_id1}/default')
            try:
                print(err.json())
            except:
                pass
